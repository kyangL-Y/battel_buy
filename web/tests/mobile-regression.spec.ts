...
test('移动端未登录时会按定位建议默认切到本地市场', async ({ page }) => {
  ...
  expect(locationSuggestionRequestCount).toBeGreaterThanOrEqual(1)
...
test('移动端账号绑定地区时定位和手选都不能覆盖账号市场', async ({ page }) => {
  ...
  await expect(page.getByText('账号地区已锁定，不能用定位或手选覆盖')).toBeVisible()
  await page.locator('.mobile-redesign-location').click()
  await expect(page.getByText('账号地区由租户配置锁定')).toBeVisible()
  await expect(page.getByRole('button', { name: '用定位建议地区' })).toHaveCount(0)
...
test('移动端首页分类进入行情时保留所选分类', async ({ page }) => {
  ...
  await expect(page.getByTestId('market-mobile-list')).toBeVisible()
  await expect.poll(() => requestedSummaryUrls.some((value) => {
    const summaryUrl = new URL(value)
    return summaryUrl.searchParams.get('liancai_top_category') === '水产类'
      && summaryUrl.searchParams.get('liancai_subcategory') === '鲜活水产'
  })).toBeTruthy()
