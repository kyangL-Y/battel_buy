...
async function seedAdminProcurementSession(page: Page, request: APIRequestContext) {
  ...
  }, [PROCUREMENT_AUTH_STORAGE_KEY, authSession] as const)
  return authSession
}

async function seedMockProcurementSession(page: Page) {
  ...
  await page.addInitScript(([storageKey, session]) => {
    ...
    access_token: 'mock-procurement-token',
    token_type: 'Bearer',
    expires_in: 3600,
...
test('桌面端账号绑定地区时辅助定位不会覆盖账号市场', async ({ page }) => {
  ...
  const procurementLoginCard = page.getByLabel('采购账号登录')
  await expect(procurementLoginCard).toContainText('南京')
  await page.waitForTimeout(2_000)
  expect(locationSuggestionRequestCount).toBe(0)

  await expect(page.getByRole('button', { name: '按定位选择' })).toHaveCount(0)
  await expect(procurementLoginCard).toContainText('南京')
  await expect(procurementLoginCard).not.toContainText('郑州')
  expect(locationSuggestionRequestCount).toBe(0)
})
...
test('桌面端平台后台拒绝非管理员且不写入后台会话', async ({ page }) => {
  ...
  const adminSession = await page.evaluate((storageKey) => window.localStorage.getItem(storageKey), ADMIN_AUTH_STORAGE_KEY)
...
test('桌面端无登录态从首页供应商入口进入供应商门户', async ({ page }) => {
  ...
  }, [LEGACY_AUTH_STORAGE_KEY, PROCUREMENT_AUTH_STORAGE_KEY, SUPPLIER_AUTH_STORAGE_KEY, ADMIN_AUTH_STORAGE_KEY] as const)
...
test('桌面端采购供应商管理遇到失效登录态时提示重新登录', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '供应商管理' }).first()).toBeVisible()
  await expect(page.getByTestId('procurement-supplier-login-gate')).toBeVisible()
  await expect(page.getByText('登录状态已失效，请重新登录后进入供应商管理')).toBeVisible()
  await expect(page.getByTestId('sales-landing-view')).toHaveCount(0)
})

test('桌面端采购供应商管理失效登录后仍保留供应商分区上下文', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1024 })
  await page.addInitScript((storageKey) => {
    window.localStorage.setItem(storageKey, JSON.stringify({
      access_token: 'expired-token',
  ...
  await expect(page.getByText('登录状态已失效，请重新登录后进入供应商管理')).toBeVisible()
  await expect(page.getByTestId('sales-landing-view')).toHaveCount(0)
  await expect(page).toHaveURL(/section=suppliers/)
})

test('桌面端 PC 工作台模块切换展示真实数据字段', async ({ page, request }) => {
  await page.setViewportSize({ width: 1440, height: 1024 })
  const authSession = await seedAdminProcurementSession(page, request)
  const summaryWarmup = await request.get(`${API_BASE_URL}/api/market/summary`, {
  ...
  await expect(page.getByText(/\d+ 个来源正在更新价格。/)).toBeVisible()
  await expect(page.getByRole('heading', { name: '来源健康巡检' })).toBeVisible()
  await expect(page.getByText('价格记录').first()).toBeVisible()
...
test('桌面端采购账号进入独立供应后台时保留团队供应商会话', async ({ page }) => {
  ...
  const mirroredSession = await page.evaluate((storageKey) => window.localStorage.getItem(storageKey), SUPPLIER_AUTH_STORAGE_KEY)
...
test('桌面端汇总行情在重型汇总接口未返回前用商品选项先展示首屏数据', async ({ page, request }) => {
  await page.setViewportSize({ width: 1440, height: 1024 })
  await seedAdminProcurementSession(page, request)
  ...
  await expect(page.getByTestId('pcw-summary-data-row').first()).toContainText('伊利淡奶油1L*6盒')
...
test('桌面端汇总行情选择莲菜网和干调类后请求后端筛选并展示数据', async ({ page }) => {
  ...
  await page.route('**/api/market/summary**', async (route) => {
    ...
    const items = sourceName === '莲菜网' && topCategory === '调味品'
      ? dryRows
  ...
  await page.getByRole('menuitemradio', { name: '调味品' }).click()
  ...
  expect(requestedSummaryUrls.some((url) => url.includes('source_name=%E8%8E%B2%E8%8F%9C%E7%BD%91') && url.includes('liancai_top_category=%E8%B0%83%E5%91%B3%E5%93%81'))).toBeTruthy()
})
...
test('桌面端汇总行情渲染前强制匹配来源和分类', async ({ page }) => {
  ...
  await expect(page.getByTestId('pcw-summary-data-row')).not.toContainText('雪天牌加碘精纯盐400g*5袋')
})

test('桌面端 PC 工作台关键按钮打开真实详情面板', async ({ page, request }) => {
  await page.setViewportSize({ width: 1440, height: 1024 })
  await seedAdminProcurementSession(page, request)
  await page.goto('/?mode=workspace&tab=summary', { waitUntil: 'domcontentloaded' })
  await expect(page.getByTestId('pc-price-workbench')).toBeVisible({ timeout: 15_000 })
...
test('桌面端 PC 工作台下拉菜单、消息和价格预警设置可操作', async ({ page, request }) => {
  await page.setViewportSize({ width: 1440, height: 1024 })
  await seedAdminProcurementSession(page, request)
  await page.goto('/?mode=workspace&tab=summary', { waitUntil: 'domcontentloaded' })
  await expect(page.getByTestId('pc-price-workbench')).toBeVisible({ timeout: 15_000 })
  ...
  const markDoneButton = firstAlertActions.getByRole('button', { name: '标记已处理' })
  if (await markDoneButton.count()) {
    ...
    await expect(page.getByText('今天没有需要你立即处理的价格提醒。').first()).toBeVisible()
  }

  await page.getByRole('button', { name: /^消息/ }).click()
  await expect(page.getByTestId('pcw-message-panel')).toBeVisible()
  await expect(page.getByTestId('pcw-message-panel')).toContainText('业务提醒')
  ...
  await expect(page.getByText('价格提醒已保存')).toBeVisible()
...
test('桌面端菜单提交后在右侧展示食材拆分结果', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1024 })
  await seedMockProcurementSession(page)
...
test('桌面端菜单采购待确认行可确认并带上下文跳转补价', async ({ page }) => {
  ...
  await expect(actionCell.getByRole('button', { name: '看菜价' })).toBeVisible()
...
test('桌面端管理员可登录并进入供应商管理台', async ({ page }) => {
  ...
  await expect(page.getByTestId('auth-session-status')).toContainText('系统管理员')
  await expect(page.getByTestId('auth-session-status')).toContainText('可管理所有供应商')
  await expect(page.getByTestId('supplier-admin-panel')).toBeVisible()
