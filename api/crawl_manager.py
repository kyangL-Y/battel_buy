...
from utils.source_config import filter_enabled_sources, get_source_name
...
class CrawlManager:
    ...
    def trigger_run(self, source: str = "manual") -> tuple[bool, dict[str, Any]]:
        with self._lock:
    ...
    def _scheduler_loop(self) -> None:
        while not self._stop_event.is_set():
            ...
            signature = (enabled, interval_seconds, fetch_mode)
            ...
            if enabled and not is_running and next_run_at and _now() >= next_run_at:
                self.trigger_run("schedule")

            self._stop_event.wait(1.0)

    def _run_crawl_job(self, source: str) -> None:
        started_at = _now()
        last_error = ""
        success_count = 0
        failed_count = 0
        ...
        try:
            ...
            products = filter_enabled_sources(products)
