...
def _load_default_env_files(base_dir: Path) -> None:
    merged_file_values: dict[str, str] = {}
    for filename in (".env.local", ".env.mysql"):
        merged_file_values.update(_parse_env_file(base_dir / filename))
    for key, value in merged_file_values.items():
        if key not in os.environ:
            os.environ[key] = value


_load_default_env_files(BASE_DIR)
