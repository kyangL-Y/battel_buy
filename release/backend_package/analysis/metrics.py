...
LOCAL_COMPARE_COLUMN_ALIASES = {
    "group_name": ["group_name", "商品分组", "分组", "商品组", "商品分组名称", "序号"],
...
def standardize_local_compare_file(df: pd.DataFrame) -> pd.DataFrame:
    ...
    text_columns = ["group_name", "product_name", "category", "brand", "product_series", "spec_text", "site_name", "compare_key"]
...
def compute_group_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    metrics = (
        base.groupby(["price_identity_key", "price_identity_label", "group_name", "brand", "product_series", "spec_text", "site_name"], dropna=False)["current_price"]
        .agg(["min", "max", "mean", "count"])
        .reset_index()
    )
    metrics["volatility"] = ((metrics["max"] - metrics["min"]) / metrics["mean"]).round(4)
    metrics["trend"] = metrics.apply(
        lambda row: detect_trend(base, row["price_identity_key"], row["site_name"], "price_identity_key", "current_price"),
        axis=1,
    )
...
def compute_cross_site_price_summary(
    ...
    latest = _build_price_identity_frame(summarize_latest_prices(df)).dropna(subset=["current_price"])
    if latest.empty:
        return pd.DataFrame()
...
def build_single_product_selector_options(
    ...
    selector_df = (
        ...
        .sort_values(["best_location_priority", "latest_captured_at", "price_identity_label"], ascending=[True, False, True])
...
def _filter_single_product_history(df: pd.DataFrame, identity_key: str) -> pd.DataFrame:
    ...
    cross_site_matches = cross_site_base[
        (cross_site_base["cross_site_identity_key"] == identity_key) & cross_site_base["current_price"].notna()
    ].copy()
    if not cross_site_matches.empty:
        return cross_site_matches

    return base[(base["price_identity_key"] == identity_key) & base["current_price"].notna()].copy()
...
def compute_single_product_summary(
    ...
    if latest.empty:
        return {}
    lowest_row = latest.sort_values(["location_priority", "current_price", "trend_series_name"], ascending=[True, True, True]).iloc[0]
    highest_row = latest.sort_values(["location_priority", "current_price", "trend_series_name"], ascending=[True, False, True]).iloc[0]
