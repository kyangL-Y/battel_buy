...
def build_overview_signals(
    ...
    metric_df = compute_group_metrics(history_df)
    ...
    for _, row in ranked.head(max(1, int(top_n))).iterrows():
        ...
        trend_name = _derive_trend_name(trend_df, fallback=metric_index.get(identity_key, {}).get("trend"))
