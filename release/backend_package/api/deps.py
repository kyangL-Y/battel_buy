...
def get_product_history_identity_df(identity_key: str) -> pd.DataFrame:
    ...
    if latest_identity_df.empty and latest_cross_site_identity_df.empty:
        return pd.DataFrame()
    ...
    matched_product_keys = list(dict.fromkeys(item for item in matched_product_keys if item))
