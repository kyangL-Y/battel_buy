...
def _build_source_priority(row: pd.Series, ingredient_family: str) -> tuple[int, int]:
...
def _build_recommendation_reason(
    ...
    if preferred_city and row_city == preferred_city:
        return f"已综合位置、来源优先级和价格推荐，当前优先同城市场；来源策略：{source_priority_label}"
    preferred_location_province, preferred_location_city = _normalize_location_preference(preferred_location)
    if preferred_location_city and row_city == preferred_location_city:
        return f"已综合位置、来源优先级和价格推荐，当前优先当前位置附近同城市场；来源策略：{source_priority_label}"
    if preferred_province and row_province == preferred_province:
        return f"已综合位置、来源优先级和价格推荐，当前优先同省市场；来源策略：{source_priority_label}"
    if preferred_location_province and row_province == preferred_location_province:
        return f"已综合位置、来源优先级和价格推荐，当前优先当前位置附近同省市场；来源策略：{source_priority_label}"
    return f"已综合位置、来源优先级和价格推荐；来源策略：{source_priority_label}"
